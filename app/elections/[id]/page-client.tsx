...
import { useElectionDetail } from "@/lib/hooks/use-dashboard-data";
...
export default function ElectionDetailContent({ id }: { id: string }) {
  const { selectedGroupId } = useViewpointGroup();
  const {
    data: election,
    isLoading,
    error,
  } = useElectionDetail(id, selectedGroupId);
